...
if __name__ == '__main__':
    ...
    parser.add_argument('--img-size', type=int, default=416, help='inference size (pixels)')  # 640
    ...
    parser.add_argument('--save-txt', type=bool, default=True, help='save results to *.txt')  # action='store_true'
    parser.add_argument('--save-conf', type=bool, default=True, help='save confidences in --save-txt labels')  # action='store_true'
    ...
    with open(os.path.join(save_dir, "result.txt"), 'w') as out:
        ...
        cla = []
        conf = []
        for txt in [f for f in os.listdir(labels) if f.endswith('txt')]:
            txt = os.path.join(labels, txt)
            read = open(txt, 'r')
